...
async function getUsers() {
    ...
    if (!functionCalls || functionCalls.length === 0) {
        return []; // Trả về mảng rỗng nếu không có giao dịch nào
    }
...
async function add_IOU(creditor, amount) {
	try {
		let signer = await provider.getSigner(); // Lấy signer chính xác
		let contractWithSigner = new ethers.Contract(contractAddress, abi, signer);
		let tx = await contractWithSigner.add_IOU(creditor, amount, { gasLimit: 300000 });
		await tx.wait();

		console.log(`IOU added: ${creditor} is owed ${amount} ETH`);
	} catch (error) {
